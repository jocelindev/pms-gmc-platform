...
def seed_database(conn: sqlite3.Connection, data: dict) -> None:
    ...
    cur.execute(
        """
        INSERT INTO app_metadata (key, value, updated_at)
        VALUES ('database_name', 'Palladium Africa Hub central', CURRENT_TIMESTAMP)
        ON CONFLICT(key) DO UPDATE SET value = excluded.value, updated_at = CURRENT_TIMESTAMP
        """
    )
    ...
    upsert_user(cur, "Administrateur Hub central", profile_ids["Administrateur"], DEFAULT_ADMIN_PASSWORD)
